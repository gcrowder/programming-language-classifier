...
public class JavaSound extends JPanel implements ChangeListener, Runnable {
	...
	// if we're an applet, add menu bar here
	// otherwise, add to frame in main() so we can use screen menu bar
        if (JavaSoundApplet.applet != null) {
	...
	// if we're not an applet, add menu bar so we can use screen menu bar
	if (JavaSoundApplet.applet == null) {
	    f.setJMenuBar(demo.getMenuBar());
	}
